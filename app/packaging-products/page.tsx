...
export default function PackagingProductsPage() {
  ...
  return (
    <div className="min-h-screen bg-offwhite text-dark font-sans">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
            {categories.slice(1).map((category, index) => (
              <button
                ...
                className={`p-4 rounded-xl border-2 transition-all duration-200 text-center font-medium text-steel bg-white border-steel hover:bg-kraft/10 hover:text-deepgreen ${selectedCategory === category.id ? 'bg-kraft/20 text-deepgreen border-deepgreen' : ''}`}
              >
      ...
      {/* Filters and Search */}
      <div className="mb-12">
        <div className="max-w-7xl mx-auto bg-white border border-steel rounded-2xl p-6">
          <div className="space-y-6">
            {/* Search */}
            <div className="relative max-w-md mx-auto">
              <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-steel" />
              <Input
                placeholder="Search products, features, specifications..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pl-12 bg-offwhite border border-steel text-dark placeholder:text-steel rounded-xl"
              />
            </div>
            {/* Categories */}
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map((category) => (
                  <button
                    key={category.id}
                    onClick={() => setSelectedCategory(category.id)}
                    className={`px-4 py-2 rounded-xl transition-all duration-200 text-sm font-medium border border-steel bg-white text-steel hover:bg-kraft/10 hover:text-deepgreen ${selectedCategory === category.id ? 'bg-kraft/20 text-deepgreen border-deepgreen' : ''}`}
                  >
              ...
              <div className="flex flex-wrap gap-2">
                {sectors.map((sector) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl transition-all duration-200 text-sm font-medium border border-steel bg-white text-steel hover:bg-kraft/10 hover:text-deepgreen ${selectedSector === sector.id ? 'bg-kraft/20 text-deepgreen border-deepgreen' : ''}`}
                  >
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`p-2 rounded-lg transition-all duration-200 ${viewMode === 'grid' ? 'bg-kraft/20 text-deepgreen' : 'bg-white text-steel'}`}
                >
                  <Grid className="w-5 h-5" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-2 rounded-lg transition-all duration-200 ${viewMode === 'list' ? 'bg-kraft/20 text-deepgreen' : 'bg-white text-steel'}`}
                >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <button
                ...
                className="text-center p-6 bg-offwhite border border-kraft/30 rounded-2xl hover:bg-kraft/10 transition-all duration-200 transform hover:scale-105"
              >
                <div className="text-4xl mb-4">{solution.icon}</div>
                <h4 className="text-xl font-bold text-deepgreen mb-3">{solution.industry}</h4>
